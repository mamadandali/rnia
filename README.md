...
class UARTCommunicator:
    ...
    def send_gh_config(self, gh_id: int, config: Dict[str, Any]):
        ...
        try:
            ...
            # Handle pre-infusion - if it's a dict, check enabled flag, otherwise treat as time value
            preinf_data = config.get('pre_infusion', {})
            if isinstance(preinf_data, dict):
                preinf = int(round(preinf_data.get('time', 0))) if preinf_data.get('enabled', False) else 0
            else:
                # If it's just a number, use it directly (for backward compatibility)
            ...
            print(f"Pre-infusion: {preinf}s (enabled: {preinf > 0})")
